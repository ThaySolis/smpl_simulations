...
// Descrição resumida: Trabalho 2 - VCube V2
...
int main(int argc, char* argv[]) {
	...
	printf("=============================================================================\n"); 
	printf("Inicio da execucao: VCube v.2 - Sistemas Distribuídos.\n");
	printf("Thayse Marques Solis\n");
	printf("Data da última modificação: 29 de janeiro de 2021\n");
	printf("Trabalho 2 - VCube v.2\n");
	printf("=============================================================================\n"); 
	...
	if(argc!=3) {
		puts("Uso correto: VCubeV2 <número de processos> <tempo de simulação>");
		exit(1);
	}
	...
	//Log1
	//Usar N = 6
	//Usar tempo de simulação > 571
	schedule(fault, 31.0, 1); // escalona uma falha no tempo 31 para o processo 1
	schedule(fault, 121.0, 2); // escalona uma falha no tempo 121 para o processo 2
	schedule(recovery, 211.0, 2); // escalona uma recuperação no tempo 211 para o processo 2
	schedule(fault, 271.0, 4); // escalona uma falha no tempo 271 para o processo 4
	schedule(recovery, 391.0, 4); // escalona uma recuperação no tempo 391 para o processo 4
	schedule(recovery, 481.0, 1); // escalona uma recuperação no tempo 481 para o processo 1
	
	
	//Log2
	//Usar N = 32
	//Usar tempo de simulação > 930
	/*schedule(fault, 31.0, 1); // escalona uma falha no tempo 31 para o processo 1
	schedule(fault, 182.0, 2); // escalona uma falha no tempo 182 para o processo 2
	schedule(recovery, 331.0, 2); // escalona uma recuperação no tempo 331 para o processo 2
	schedule(fault, 481.0, 4); // escalona uma falha no tempo 601 para o processo 4
	schedule(recovery, 631.0, 4); // escalona uma recuperação no tempo 800 para o processo 4
	schedule(recovery, 781.0, 1); // escalona uma recuperação no tempo 901 para o processo 1*/
	...
	while(time()<=tempo_simulacao) {
		...
		int num_proximo_processo = num_processo_atual; // se continuar com este valor, quer dizer que nao tem nenhum outro nodo ok.
		switch(event) {
			case test:
			//printf("#####O processo %d vai testar no cluster %d\n", num_processo_atual, processo[num_processo_atual].cluster_atual);
				...
				printf("\n\n");
				//Quando o processo num_processo_atual vai testar:
				for (int y = 0; y < N; y ++) //Para cada processo "y"
				{
					node_set* testadores_de_y;
					testadores_de_y = cis(y, processo[num_processo_atual].cluster_atual);
					
					//Verifica se num_processo_atual está em testadores_de_y.
					for (i = 0; i < testadores_de_y->size; i++)
					{
						if (testadores_de_y->nodes[i]==num_processo_atual)
						{
							conta_testes++;
							//printf("#####O processo %d vai testar o processo %d\n", num_processo_atual, y);
							if (status(processo[y].id) != 0) { 
								printf("Tempo %4.1f: TESTE: o processo %d testou %d que está falho.\n", time(), num_processo_atual, y);
								// o processo num_processo_atual acabou de testar o processo y, agora ele vai atualizar o vetor State dele próprio (o num_processo_atual).
								if (processo[num_processo_atual].State[y]==-1)
								{
									processo[num_processo_atual].State[y]=1;
								}
								if (eh_par(processo[num_processo_atual].State[y]))
								{
									processo[num_processo_atual].State[y]++;
								}
							}
							else{
								printf("Tempo %4.1f: TESTE: o processo %d testou %d que está correto.\n", time(), num_processo_atual, y);
								if (!eh_par(processo[num_processo_atual].State[y]))
								{
									processo[num_processo_atual].State[y]++;
								}
								// Agora o processo num_processo_atual tem que aprender o estado global com o processo y
								for (int num_outro_processo = 0; num_outro_processo < N; num_outro_processo++) {
									if (num_outro_processo!=num_processo_atual)
									{
										if (processo[y].State[num_outro_processo]>processo[num_processo_atual].State[num_outro_processo]) {// Se o processo atual tem estados salvos mais antigos (menores) que o do vetor de estados do processo testado correto, ele aprende com o processo testado correto
											processo[num_processo_atual].State[num_outro_processo]=processo[y].State[num_outro_processo];
											printf("\tO processo %d aprendeu com o processo %d que o estado do processo %d é %d\n", num_processo_atual, y, num_outro_processo, processo[y].State[num_outro_processo]);
						...
						else if (testadores_de_y->nodes[i] < N && eh_par(processo[num_processo_atual].State[testadores_de_y->nodes[i]])) 
						{
							...
							//printf("#####O processo %d não vai testar o processo %d pois acha que o %d vai fazer isso\n", num_processo_atual, y, testadores_de_y->nodes[i]);
							break;
						}
					}
